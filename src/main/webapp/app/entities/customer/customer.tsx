  ...
  return (
    <div>
      <h2 id="customer-heading">
        <Translate contentKey="abddApp.customer.home.title">Customers</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="abddApp.customer.home.createLabel">Create new Customer</Translate>
        </Link>
      </h2>
        ...
        {customerList && customerList.length > 0 ? (
            ...
            <tbody>
              {customerList.map((customer, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {customer.dateBecameCustomer ? (
                      <TextFormat type="date" value={customer.dateBecameCustomer} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>{customer.mobileNo}</td>
                  <td>
                    {customer.address ? <Link to={`address/${customer.address.id}`}>{customer.address.line1BuildingNumber}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="abddApp.customer.home.notFound">No Customers found</Translate>
            </div>
          )
        )}
      </div>
