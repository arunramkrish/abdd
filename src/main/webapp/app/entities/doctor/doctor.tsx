  ...
  return (
    <div>
      <h2 id="doctor-heading">
        <Translate contentKey="abddApp.doctor.home.title">Doctors</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="abddApp.doctor.home.createLabel">Create new Doctor</Translate>
        </Link>
      </h2>
        ...
        {doctorList && doctorList.length > 0 ? (
            ...
            <tbody>
              {doctorList.map((doctor, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{doctor.address ? <Link to={`address/${doctor.address.id}`}>{doctor.address.line1BuildingNumber}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="abddApp.doctor.home.notFound">No Doctors found</Translate>
            </div>
          )
        )}
      </div>
